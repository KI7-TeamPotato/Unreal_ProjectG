...
public:
    ...
    UUnitUIDataAsset* RollSingleGacha(const TArray<UUnitUIDataAsset*>& GachaPool, float NormalRate = 75.0f, float RareRate = 20.0f, float SuperRareRate = 5.0f);

    // 연속 뽑기(10연뽑 등): 지정한 횟수만큼 뽑아 배열로 반환
    UFUNCTION(BlueprintCallable, Category = "UnitCollection|Gacha")
    TArray<UUnitUIDataAsset*> RollMultiGacha(const TArray<UUnitUIDataAsset*>& GachaPool, int32 PullCount = 10, float NormalRate = 75.0f, float RareRate = 20.0f, float SuperRareRate = 5.0f);
